...
#         pattern9 = re.compile(r'<dt><dt>联系人：</dt><dd>(.*?)</dd>', re.S)
...
#                 item["linkman"] = "".join(re.findall(pattern2,html_text)).split(' ')[0].replace(' ','')
...
#                 else:
#                     try:
#                         item["linkman"] = "".join(re.findall(pattern9, html_text))
#                         if item["linkman"]:
#                             item["linkman"] = self.search_linkman(item["linkman"])
#                     except:
#                         item["linkman"] = ''
...
#                 else:
#                     try:
#                         item["company_address"] = "".join(re.findall(pattern7, html_text)).replace('</a> ', '')
#                         if item["company_address"]:
#                             item["company_address"] = self.search_address(item["company_address"])
#                     except:
