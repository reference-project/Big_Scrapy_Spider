...
class SjgcSpider(scrapy.Spider):
    ...
    custom_settings = {
        ...
        # "DOWNLOAD_HANDLERS": {
        #     'https': 'shijiegongchang.custom.downloader.handler.https.HttpsDownloaderIgnoreCNError',
        # },
    }
    ...
    def parse_company_contact(self, response):
        ...
        if item["company_address"]:
            ...
            try:
                item["company_address"] = "".join(re.findall(pattern,response.text)) if re.findall(pattern,response.text) else ''
            except:
    ...
    def search_phone_num(self, text):
        if text:
            try:
                ...
                text = re.sub(r'\s|\r|\t|\n|保密|移动电话：|电话：|未填写|无|联系电话|手机', '', text).strip()
    ...
    def search_telephone_num(self, text):
        if text:
            try:
                ...
                text = re.sub(r'\s|\r|\t|\n|公司电话：|：|暂未提供|未填写|无|联系电话', '', text).strip()
                text = "".join(re.findall(pattern, text))
                return text
            except:
                return ''
    ...
    def search_linkman(self, text):
        if text:
            ...
            text = re.sub(r'\s|\r|\n|\t|联系人：|公司联系人：|：|暂未提供|未填写|无|联系人', '', text)
            try:
                text = text.split('(')[0]
                if text:
                    if len(text) > 3:
                        text = re.sub(r'\s|\r|\n|\t|先生|女士|小姐|总经理|总监|技术总监', '', text)
                        return text
                    else:
                        return text
                else:
